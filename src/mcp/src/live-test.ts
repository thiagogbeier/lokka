...
async function testLiveGraphRequest() {
  ...
  try {    const client = new SimpleMCPClient(serverProcess);
    ...
    const tokenResponse: any = await client.callTool('set-access-token', {
    ...
    const statusResponse: any = await client.callTool('get-auth-status', {});
    console.log("✅ Auth status checked");

    console.log("\n📡 Step 5: Make live Microsoft Graph request (/me)");
    const graphResponse: any = await client.callTool('Lokka-Microsoft', {
      apiType: 'graph',
      path: '/me',
      method: 'get'
    });
    ...
    const groupsResponse: any = await client.callTool('Lokka-Microsoft', {
      apiType: 'graph',
      path: '/me/memberOf',
      method: 'get',
      queryParams: {
