...
<details>
<summary> (Optional) Grant access to Azure (click to expand)</summary>

If you want to use the agent with Azure you need to grant access to the Azure resources.

For each subscription you want to use the agent with, you need to assign at least the `Reader` role to the application.

- Open [Azure portal](https://portal.azure.com) > **Subscriptions**
- Select the subscription you want to grant access to
- Select **Access control (IAM)** > **Add role assignment** > **Reader** > **Next**
- Select **Select members** and search for the application you created earlier (e.g. `Lokka`)
- Select **Review + assign** twice to confirm
- Repeat this for each subscription you want to use the agent with.
</details>
